...
pub fn bounded_parralel_map<I, F, T, U>(bound: usize, iter: I, f: F) -> Receiver<U>
where
...
where
    ...
    U: Send + 'static,
{
    bounded_parralel_map(bound, iter, f).into_iter()
}

pub fn bounded_parralel_map_channel<T, U, F>(bound: usize, f: F) -> (SyncSender<T>, Receiver<U>)
where
    F: Fn(T) -> U,
    F: Send + Sync + 'static,
    T: Send + 'static,
    U: Send + 'static,
{
    ...
    (
        source_tx,
        bounded_parralel_map(bound, source_rx.into_iter(), f),
    )
}
...
mod tests {
    ...
    use crate::{
        bounded_parralel_map_channel, bounded_parralel_map_channel_scope,
        bounded_parralel_map_iterator, bounded_parralel_map_iterator_scope,
    };

    #[test]
    fn test_bounded_parralel_map_channel() {
        let result_rx = {
            let (source_tx, result_rx) = bounded_parralel_map_channel(2, |x: u64| {
    ...
    fn test_bounded_parralel_map_iterator() {
        ...
        let result: Vec<i32> =
            bounded_parralel_map_iterator(3, sut.into_iter(), |x| x * x).collect();
        assert!(result == vec![1, 4, 9, 16, 25, 36, 49]);
    }

    #[test]
    fn test_bounded_parralel_map_iterator_scope() {
        scope(|s| {
            let sut = vec![1, 2, 3, 4, 5, 6, 7];
