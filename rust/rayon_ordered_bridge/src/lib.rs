...
mod tests {
    ...
    use crate::{bounded_parralel_map_channel, bounded_parralel_map_iterator};

    #[test]
    fn test_bounded_parralel_map_channel() {
        scope(|s| {
            let result_rx = {
                let (source_tx, result_rx) = bounded_parralel_map_channel(s, 2, |x: u64| {
    ...
    fn test_bounded_parralel_map_iter() {
        scope(|s| {
            ...
            let result: Vec<i32> =
                bounded_parralel_map_iterator(s, 3, sut.into_iter(), |x| x * x).collect();
